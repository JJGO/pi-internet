 ...
 *
 * Provenance: pi-web-access/rsc-extract.ts (copied verbatim)
 * This parser handles Next.js pages that serve RSC flight data instead of
 * rendered HTML. Readability returns empty on these — this parser extracts
 * the actual content from the $L-referenced chunk tree.
 */
...
export function extractRSCContent(html: string): RSCExtractResult | null {
  ...
  // Next.js commonly serializes the main page content in chunk "23" (hex 0x17).
  // This is an empirical heuristic, not a spec guarantee.
  // The fallback loop below handles other chunk IDs if this misses.
